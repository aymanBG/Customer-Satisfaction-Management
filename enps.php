...
<div class="dropdown d-flex justify-content-center" >
  ...
  <div id="myDropdown" class="dropdown-content">
    ...
    <?php
        $usc = $conn->query("SELECT s.* , u.id, u.middlename from users as u, survey_set as s where FIND_IN_SET(s.title,u.serv) and s.title = 'E-NPS'");
        while ($row = $usc->fetch_assoc()) :
        ?>
        
      
            <a href="http://localhost/mxp/index.php?page=enps&selected=<?php echo ucwords($row['id'])  ?>" ><?php echo ucwords($row['middlename'])  ?></a>
...
<form method="POST">
  ...
  <input name="reviewdate" type="text" id="datepicker" />
    <input name="enddate" type="text" id="datepickere" />
...
$tot = $conn->query("SELECT a.survey_id, q.frm_option, a.user_id, a.answer from answers as a, questions as q where a.question_id = q.id and a.survey_id = 18 and a.user_id = '" . $select . "'  ORDER BY user_id desc")->num_rows;
$rst = $conn->query("SELECT a.survey_id,a.question_id, Month(a.date_created) as time, q.frm_option, a.user_id,COUNT(a.answer) as t,a.answer from answers as a, questions as q where a.question_id = q.id and a.survey_id = 18  and a.date_created between '".$no."' AND '".$on."' and a.user_id = '" . $select . "' GROUP by a.answer");
$ans = array();
$sp = 0;
$sr = 0;
$sd = 0;
...
//query to get data from the table
$tot = $conn->query("SELECT a.survey_id, q.frm_option, a.user_id, a.answer from answers as a, questions as q where a.question_id = q.id and a.survey_id = 18 and a.user_id = '" . $select . "' and answer <> '' ORDER BY user_id desc")->num_rows;
$rst = $conn->query("SELECT a.date_created, COUNT(a.answer) as t,a.answer from answers as a, questions as q where a.question_id = q.id and a.survey_id = 18 and a.user_id = '" . $select . "' and a.answer != 'qsGRh' AND a.answer !='dntHO' and a.answer !='lChTn' and a.answer !='BWjkU' GROUP by EXTRACT(YEAR_MONTH FROM a.date_created) order by a.date_created asc;");
